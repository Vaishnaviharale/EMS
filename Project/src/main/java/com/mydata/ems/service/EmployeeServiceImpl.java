...
new ResourceNotFoundException("employee is not exist with given id:"+empId));
return EmployeeMapper.mapToEmployeeDto(emp);
}
@Override
public List<EmployeeDto> getAllEmployees() {
List<Employee> allEmp=employeeRepository.findAll();
return allEmp.stream().map((Employees)->EmployeeMapper.mapToEmployeeDto(Employees)).collect(Collectors.toList());
